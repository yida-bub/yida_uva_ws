...
#define YOLO_RUNS_DETECT_PATH "yolov5/runs/detect/"
...
int confirm_torget_data(data_t* new_data, data_t* buf_data, data_t* olddata, int const buf_data_len){
	...
	for(int i = 0; i < buf_data_len-1 && buf_data[i].classes == 0; i++){
		// new_min_dist = sqrt(pow(fabs(olddata->x_point-buf_data[i].x_point - olddata->x_point-buf_data[i+1].x_point),2) + pow(fabs(olddata->y_point-buf_data[i].y_point - olddata->y_point-buf_data[i+1].y_point),2));
