...
data_t getdata(char* const path, int const file_num, int const camere_num){
	//组织数据文件的路径
	char data_path[256] = {};
...
int main(int argc, char* argv){
	...
	data_t data = getdata(path, file_num, camere_num);
	//数据换算
	//控制 x的速度和转向貌似就够
	...
	if(data.y_point-k > -Y_OVERLOOK_K && data.y_point-k < Y_OVERLOOK_K){ //目标在图像中心一定范围内，无动作
		move.x_vel = 0;
	}else if(data.y_point-k > Y_OVERLOOK_K){ //目标在图像中心的下方，后退
		move.x_vel = -0.5;
	}else if(data.y_point-k < -Y_OVERLOOK_K){ //目标在图像中心的上方，前进
		move.x_vel = 0.5;
	}
	#define X_OVERLOOK_K 0.20 //图像中心点x坐标无动作范围
	if(data.x_point-k > -X_OVERLOOK_K && data.x_point-k < X_OVERLOOK_K){ //目标在图像中心的一定范围内，无动作
		move.yaw = 0.0;
	}else if(data.x_point-k > X_OVERLOOK_K){ //目标在图像中心的右边，顺序针偏航
		move.yaw = 1.0;
	}else if(data.x_point-k < -X_OVERLOOK_K){ //目标在图像中心的左边，逆时针偏航
		move.yaw = -1.0;
	}
